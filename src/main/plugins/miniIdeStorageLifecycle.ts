...
interface LifecycleRecord {
  pluginId: typeof MINI_IDE_PLUGIN_ID
...
export interface MiniIdeStorageLifecycleFileOps {
...
const defaultFileOps: MiniIdeStorageLifecycleFileOps = {
  mkdirSync,
  openSync,
  writeSync,
  fsyncSync,
 ...
 */
export class UnreadableMiniIdeLifecycleRecordError extends Error {
  constructor(readonly reason: unknown) {
    super(
      'MiniIde storage lifecycle record is unreadable; reset the record to recover without reinstalling MiniIde',
    )
    this.name = 'UnreadableMiniIdeLifecycleRecordError'
  }
}

/**
 * Records the exact active MiniIde package identity. Upgrade recovery consumes
 * this record only; it never guesses a previous version by scanning storage
 * directories or comparing semver values.
 */
export class MiniIdeStorageLifecycleSelector {
  constructor(
    private readonly recordPath: string,
    private readonly fileOps: MiniIdeStorageLifecycleFileOps = defaultFileOps,
  ) {}
  ...
  private readRecord(): LifecycleRecord | null {
    ...
    try {
      ...
      throw new UnreadableMiniIdeLifecycleRecordError(error)
    }
    try {
      const record = JSON.parse(raw) as Partial<LifecycleRecord>
      if (
        record.pluginId !== MINI_IDE_PLUGIN_ID ||
        record.tier !== 'active' ||
      ...
      ) throw new Error('invalid MiniIde storage lifecycle record')
  ...
  sourceFor(packageVersion: string): HostStorageSnapshotIdentity | null {
    ...
    return { pluginId: MINI_IDE_PLUGIN_ID, packageVersion: sourcePackageVersion, tier: 'active' }
  }

  /** Called only by the existing explicit plugin-uninstall cleanup flow. */
  clear(): void {
    try { this.fileOps.unlinkSync(this.recordPath) }
  ...
  rememberActive(packageVersion: string, sourcePackageVersion?: string | null): boolean {
    ...
    try {
      ...
      if (
        previous.pluginId === MINI_IDE_PLUGIN_ID &&
    ...
    try {
      ...
      if (descriptor === null) throw new Error('could not create MiniIde lifecycle temporary file')
      ...
      while (offset < bytes.length) {
        ...
        if (written <= 0) throw new Error('MiniIde lifecycle write made no progress')
        offset += written
      }
      ...
      console.warn('[mini-ide-storage] lifecycle selector write failed', error)
      return false
    }
  }
}
