    ...
    <section class="ext-section">
      ...
      <ul class="ext-list">
        <li v-for="p in nonFactoryInstalled" :key="p.id" class="ext-installed" :data-id="p.id">
          ...
          <button class="ext-remove" @click="remove(p.id)">{{ $t('settings.extensions.remove') }}</button>
        </li>
...
/* This button declares nothing of its own — it is a native browser button.
 * Hover therefore darkens what the platform already painted instead of
 * replacing the fill: an overlay colour here would flip a light native button
 * to a dark one on hover, which is a bigger change than the missing feedback
 * it fixes. */
.ext-remove {
