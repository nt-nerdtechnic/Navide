...
describe('useAccountPeriods', () => {
  it('sends tokens.account_periods for one account and exposes rows and period totals', async () => {
    ...
    expect(mock.sent[0].payload).toEqual({ agent_key: 'claude', profile_id: 'slot-a', granularity: 'month' })
  ...
  it('omits the account fields for the all-accounts query', async () => {
    ...
    expect(mock.sent[0].payload).toEqual({ agent_key: undefined, profile_id: undefined, granularity: 'year' })
