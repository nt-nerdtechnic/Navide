"""crontab and launchd, the POSIX implementation of `osplat.spec.Scheduler`.

Moved verbatim from `executions_service` when the seam was introduced: the
parsers, the read-modify-write guard on the crontab, the plist scan and the
`launchctl` calls are the code macOS has always run. What changed is only
who decides whether launchd exists — `PosixScheduler(launchd=...)`, chosen by
`_darwin` (True) and `_linux` (False) — so no function here asks which
platform it is on.
...
Every external command runs through :func:`_run`, which uses
``create_subprocess_exec`` (never a shell) and always enforces a timeout, so a
hung ``crontab``/``launchctl`` cannot pin a request open forever.
"""
...
from ._posix_secrets import secret_files
from .spec import SchedulerError as ExecutionsError
...
CRONTAB_UNSUPPORTED = {"supported": False, "entries": [], "unparsed": 0, "error": None}
LAUNCHD_UNSUPPORTED = {"supported": False, "entries": [], "unreadable": 0, "error": None}


class PosixScheduler:
    """crontab everywhere; launchd only where the platform module says so."""

    def __init__(self, *, launchd: bool) -> None:
        self._launchd = launchd

    async def list_jobs(self, kind: str) -> dict:
        if kind == "crontab":
            return await list_crontab()
        if kind == "launchagent":
            if not self._launchd:
                return dict(LAUNCHD_UNSUPPORTED)
            return await list_launch_agents()
        raise ValueError(f"unknown execution kind: {kind!r}")

    async def set_enabled(self, kind: str, target: str, enabled: bool) -> None:
        if kind == "crontab":
            await set_crontab_enabled(target, enabled)
            return
        self._require_launchd(kind)
        await set_launch_agent_enabled(target, enabled)

    async def remove(self, kind: str, target: str) -> None:
        if kind == "crontab":
            await remove_crontab_entry(target)
            return
        self._require_launchd(kind)
        await remove_launch_agent(target)

    def _require_launchd(self, kind: str) -> None:
        if kind != "launchagent":
            raise ValueError(f"unknown execution kind: {kind!r}")
        if not self._launchd:
