...
Please open a [GitHub Issue](../../issues/new?template=bug_report.yml) using the bug report template.
Include a clear description, steps to reproduce, and any relevant logs or screenshots.

> 請開 [GitHub Issue](../../issues/new?template=bug_report.yml) 並填寫 bug 回報模板，包含重現步驟與相關 log。

---

## Requesting Features / 功能建議

Open a [GitHub Issue](../../issues/new?template=feature_request.yml) with the feature request template.
Describe the problem you are trying to solve and the solution you have in mind.

> 請開 [GitHub Issue](../../issues/new?template=feature_request.yml) 並描述你想解決的問題與建議的解法。
...
> 請與現有程式碼風格保持一致。Python 提交前請執行 `uv run ruff check .`。
