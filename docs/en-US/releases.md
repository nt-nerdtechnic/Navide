...
> **Current distribution status:** v0.1.44 is the latest explicitly labeled unsigned macOS arm64 preview prerelease. It provides direct GitHub downloads but is not an official signed/notarized stable release and is not selected by the stable updater channel. A future signed release must use a higher version.
