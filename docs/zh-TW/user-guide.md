...
**為帳號命名。**每個帳號都能取一個你自己的名字，內建的 Default 也可以：在「帳號」分頁滑到卡片上按鉛筆，或在額度徽章的帳號清單那一列按鉛筆。這個名字會成為各處顯示帳號時的主要文字——pane 標題列、帳號清單、卡片、切換通知與 Turn Stats——登入身分則留在它旁邊；把欄位清空就回到自動產生的名稱（Account 1、Account 2…）。對於憑證不帶身分的 CLI，你取的名字是唯一能分辨兩個帳號的方式。
