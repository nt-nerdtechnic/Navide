...
- Terminal は返信できません。MCP ツールを持たず、その出力からメッセージや Spawn
  ブロックが探されることもありません。
- **Guard は Terminal を記録するだけで、コマンドを止めることはできません。**
  メッセージは Agent と同じく Terminal を外部の影響ありとマークしますが、Guard の
  コマンド検査は CLI 自身のツールフックの中で動き、プレーンなシェルにはフックが
  ありません。テキストとシェルの間に Guard の判定は入りません。
